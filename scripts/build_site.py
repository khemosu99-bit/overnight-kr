...
live, fail = {}, []
# 대체 심볼 (주 심볼 실패 시 시도)
ALT = {"SOX_r": ["^SOX", "SOXX", "^SOXX"], "NASDAQ_r": ["^IXIC", "^NDX"],
       "SP500_r": ["^GSPC", "^SPX"], "EWY_r": ["EWY"], "USDKRW_r": ["KRW=X"]}

for f in FEAT:
    got = False
    for sym in ALT.get(f, [SYM[f]]):
        try:
            live[f] = quote(sym)
            src = "" if sym == SYM[f] else f" (대체:{sym})"
            print(f"  OK   {NAME[f]:<12} {live[f]['price']:>11,.2f}  {live[f]['pct']:+.2f}%{src}")
            got = True
            break
        except Exception:
            continue
    if not got:
        fail.append(f)
        print(f"  FAIL {NAME[f]:<12} 모든 심볼 실패")
...
# 핵심 모델 입력이 빠지면 환산 불가로 처리
CORE = ["SOX_r", "EWY_r", "NASDAQ_r"]
core_missing = [NAME[f] for f in CORE if f in FEAT and f in fail]
if core_missing and session in ("pre", "live", "post"):
    warn = f"핵심 지표({', '.join(core_missing)})를 수집하지 못했습니다"
    print(f"  ⚠️ 핵심 지표 누락: {core_missing} → 환산 보류")
...
shell("코스피200 야간선물 개장 갭 | nightgap.co.kr",
      f"간밤 야간선물 대용 미국 지표로 코스피 개장 갭을 환산합니다. "
